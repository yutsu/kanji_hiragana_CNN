...
function displayResult(results, probs) {
    ...
    for (var i = 0; i < results.length; i++) {
        $('#result-title').html('Result 結果')
        $('#jp' + (i+1)).html(results[i])
...
function barGraph(prob, i) {
    ...
    let jp = document.getElementById('jp' + (i + 1))
    jp.style.backgroundImage = 'linear-gradient(' + colors[i] +', ' +colors[i] + ')';
    jp.style.backgroundSize = Math.round(prob * 100) + '%' + ' 100%'
}
...
function getLabels(indices) {
    ...
    for (let i = 0; i < indices.length; i++)
        output[i] = japaneseCharacters[indices[i]]
    return output
}
...
function allowDrawing() {
    ...
    document.getElementById('status').innerHTML = 'Ready! 準備完了です!';
